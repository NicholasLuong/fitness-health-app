...
export function Modal({ title, children, onClose, wide = false }: { title: string; children: ReactNode; onClose: () => void; wide?: boolean }) {
  const [viewport, setViewport] = useState(visibleViewportRect)
  useEffect(() => {
    const visualViewport = window.visualViewport
    if (!visualViewport) return
    let animationFrame = 0
    let settleTimer = 0
    const update = () => setViewport(visibleViewportRect())
    const sync = () => {
      update()
      ...
      animationFrame = window.requestAnimationFrame(() => {
        update()
        settleTimer = window.setTimeout(update, 100)
      })
    }
  ...
  return <div className="modal-backdrop" role="presentation" style={{ top: viewport.top, height: viewport.height }} onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
